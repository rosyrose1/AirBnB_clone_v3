...
'''Permissible methods for the amenities endpoint.'''
...
def handle_amenities(amenity_id=None):
    '''Function to handle requests to the amenities endpoint.
    '''
    handlers = {
        'GET': get_amenities,
...
def get_amenities(amenity_id=None):
    '''Retrieves the amenity with the specified id or all amenities.
    '''
...
def remove_amenity(amenity_id=None):
    '''Deletes an amenity with the specified id.
    '''
    all_amenities = storage.all(Amenity).values()
    res = list(filter(lambda x: x.id == amenity_id, all_amenities))
    if res:
...
def update_amenity(amenity_id=None):
    '''Modifies the amenity with the specified id.
    '''
