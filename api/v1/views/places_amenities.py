#!/usr/bin/python3
'''Holds the places_amenities view for the API.'''
...
def handle_places_amenities(place_id=None, amenity_id=None):
    '''The function that deals the places endpoint.
    '''
    handlers = {
...
def get_place_amenities(place_id=None, amenity_id=None):
    '''Retrieves the amenities of a place with the given id.
    '''
    if place_id:
...
def remove_place_amenity(place_id=None, amenity_id=None):
    '''Deletes an amenity with a given id from a place with a given id.
    '''
...
def add_place_amenity(place_id=None, amenity_id=None):
    '''Includes an amenity with a given id to a place with a given id.
    '''
    if place_id and amenity_id:
        place = storage.get(Place, place_id)
        if not place:
